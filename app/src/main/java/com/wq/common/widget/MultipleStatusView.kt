...
class MultipleStatusView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : RelativeLayout(context, attrs, defStyleAttr) {
    ...
    init {
        ...
        mEmptyViewResId = a.getResourceId(R.styleable.MultipleStatusView_emptyView, R.layout.view_multi_status_empty)
        mErrorViewResId = a.getResourceId(R.styleable.MultipleStatusView_errorView, R.layout.view_multi_status_error)
        mLoadingViewResId = a.getResourceId(R.styleable.MultipleStatusView_loadingView, R.layout.view_multi_status_loading)
        mNoNetworkViewResId = a.getResourceId(R.styleable.MultipleStatusView_noNetworkView, R.layout.view_multi_status_no_network)
    ...
    fun showEmpty(view: View, layoutParams: ViewGroup.LayoutParams) {
        ...
        if (null == mEmptyView) {
            mEmptyView = view
            val emptyRetryView = view.findViewById(R.id.empty_retry_view) as View?
    ...
    fun showError(view: View, layoutParams: ViewGroup.LayoutParams) {
        ...
        if (null == mErrorView) {
            mErrorView = view
            val errorRetryView = view.findViewById(R.id.error_retry_view) as View?
    ...
    fun showNoNetwork(view: View, layoutParams: ViewGroup.LayoutParams) {
        ...
        if (null == mNoNetworkView) {
            mNoNetworkView = view
            val noNetworkRetryView = mNoNetworkView!!.findViewById(R.id.no_network_retry_view) as View?
    ...
    fun showContent() {
        ...
        if (null == mContentView && mContentViewResId != NULL_RESOURCE_ID) {
            ...
            mContentView = findViewById(mContentViewResId) ?: mInflater!!.inflate(mContentViewResId, null)
            mContentView?.parent ?: addView(mContentView, 0, DEFAULT_LAYOUT_PARAMS)
        }
        showContentView()
    }
